...
def limpiar_y_procesar_xlsx(uploaded_file: io.BytesIO) -> pl.DataFrame:
    ...
    try:
        ...
        df_pd = pd.read_excel(
            ...
            engine='openpyxl',
            sheet_name=0,
            # Forzar columna "# SET" (índice 3) como string para preservar ceros iniciales
            dtype={3: str}
        )

    except Exception as e:
        st.error(
            f"Error al intentar leer el archivo Excel: {e}")
        return pl.DataFrame()
    ...
    if df_pl.shape[1] != len(NUEVAS_CABECERAS):
        ...
        column_mapping = {old_col: new_col for old_col,
                            new_col in zip(old_col_names, NUEVAS_CABECERAS)}
        df_final = df_pl.rename(column_mapping)

        # --- B. Lógica de Relleno (Fill Null) con Polars ---
        st.info(
            "🧠 Aplicando tareas y calculando números de SETs disponibles...")
...
if uploaded_file is not None:
    ...
    try:
        ...
        if not df_procesado.is_empty():
            ...
            set_values = df_procesado.select(
                "# SET").to_series().drop_nulls().unique().to_list()
            # Filtrar códigos válidos de 8 dígitos
            valid_sets = [s for s in set_values if isinstance(
                s, str) and len(s) == 8 and s.isdigit()]
            ...
            free_sets = st.subheader(
                "Números de SET disponibles", anchor="free_sets")

            col_urbano, col_rural = st.columns(2)

            with col_urbano:
                col_dato, col_copy = st.columns([10, 1])
                with col_dato:
            ...
            st.download_button(
                ...
                # Polars escribe el CSV en memoria (buffer)
                data=df_procesado.write_csv(None),
